...
const SmallPicker = ({first_date,second_date,togglePopUp,changeShowedMonth}) => {
    ...
    return(
        <div className="small-picker">
            <i className="fas fa-calendar-alt" onClick={togglePopUp}></i>
            <div className="date" onClick={togglePopUp}>{first_date}-{second_date}</div>
            <div className="arrow-group">
                <img className="left-arrow" src={left_arrow} onClick={e => {changeShowedMonth(1)}} /> 
                <img className="left-arrow" src={right_arrow} onClick={e => {changeShowedMonth(-1)}}  /> 
            </div>
        </div>
    )
}
...
export default connect(mapStateToProps,{togglePopUp,changeShowedMonth})(SmallPicker);